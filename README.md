...
- **Fixed:** logged-in users no longer see "Sign up" on `/`, `/login`, or `/register` — these routes now redirect straight to `/app` once the session is known (new `PublicRoute` guard). Previously `/` only conditionally swapped a button, which didn't cover every case (e.g. navigating back).
- **New:** create a task directly from the Calendar page. Click a day → "Add task" → pick a board, title, priority — the due date is locked to the day you clicked. Uses the same `tasksApi.create` as everywhere else, so it shows up instantly on the relevant board too.
- **New:** a real app logo — a gradient checkmark mark (`components/common/Logo.tsx`), used in the sidebar, both auth pages, and as the favicon/app icon. See "Icons" below for how the PNGs were generated.
- **New:** light/dark toggle on the public pages (Landing, Login, Register) — previously only available once logged in. Shared `ThemeToggle` component now used everywhere instead of duplicated logic.
- **New:** installable PWA — manifest, generated icons (192/512/maskable + favicons + apple-touch-icon), and a service worker (via `vite-plugin-pwa`) that precaches the app shell for offline use. API calls are deliberately **not** cached — this app's data changes too often for that to be safe.
- **New:** task labels are now editable (comma-separated field in the task modal), not just displayed — this was a real gap, not a design choice.

## Icons

`public/icons/*.png`, `public/favicon-*.png`, and `public/apple-touch-icon.png` were generated by `scripts/generate-icons.cjs` — a small pure-JS rasterizer (using `pngjs`, no native image libraries) that draws the gradient-and-checkmark mark at each required size. They're already committed, so you don't need to run anything. If you want to regenerate them (e.g. after changing the logo), run:

```bash
npm install pngjs --no-save
